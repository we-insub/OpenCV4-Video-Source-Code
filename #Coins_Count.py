...
# 입력 이미지 불러오기
src = cv2.imread('/home/mato/Downloads/ch06/coins1.jpg')
...
# 칼라 스페이스를 그레이로 변경 원의 크기를 구할땐 RGB를 쓸 필요없기때문에 그레이로 변경
...
if circles is not None:
    for i in range(circles.shape[1]):
        ...
        hsv = cv2.cvtColor(crop, cv2.COLOR_BGR2HSV) #HSV로 칼라
        hue, _, _ = cv2.split(hsv) # 채널에서 휴만 가지고오려고 스플릿 함
        hue_shift = (hue + 40) % 180 # Hue의 값은 360도 지만 256 안으로집어넣어야함
        #그렇기 떄문에 179까지 표현을 하기 때문에 180%으로 나눴다는 색이 어디로 표현해있는지를
        #알기위해서 이다.
